...
private: 
    ...
    int linear_t_, linear_b_,angular_; 
    
    // used to define which axes of the joystick will control our turtle 
...
{ 
    ...
    nh_.param("axis_linear_t", linear_t_, linear_t_); 
    nh_.param("axis_linear_b", linear_b_, linear_b_);
...
{ 
    ...
    // take the data from the joystick and manipulate it by scaling it and using independent axes to control the linear and angular velocities of the turtle 
     ...
     if (abs(l_scale_*joy->axes[linear_b_])!=0)
     {
         /* code for True */
         twist.linear.x = l_scale_*joy->axes[linear_b_]-1;
     }
     else if(abs(( l_scale_*joy->axes[linear_t_]))!=0)
            {
                twist.linear.x =-(( l_scale_*joy->axes[linear_t_])-22);
            }
